...
**This does not create component sets.** `ds-figma-component` is the skill for that, but it is
currently **BLOCKED** — its own input directory no longer exists (see its banner). This skill takes
a set that already exists — generated by us or hand-built by a designer — and makes the page around
it readable.
...
| You want                                           | Skill                                                                                     |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| A component set that does not exist yet            | `ds-figma-component` — **but it is blocked today; there is no working path to a new set** |
| The page around a set: description, labels, states | **this one**                                                                              |
| To reorganise several component pages              | **this one**                                                                              |
| To add a variant axis to a set                     | `ds-figma-component` — same blocker applies                                               |
...
- **Contracts exist, but no working skill builds a Figma set from them.**
  `packages/contracts/components/` now contains component contracts, while `.figma/maps/components.json`
  has no entries. `ds-component` (code authoring) is superseded, and `ds-figma-component` (Figma set
  generation) is separately **BLOCKED** — both target `packages/react/src/components/`, which no
  longer exists. So this skill can only document sets that already exist in the Figma file on their
  own terms; it has no working companion to hand a "build one first" request to.
- **No theme axis.** `.figma/manifest.json` → `identity.themes` records `modes: ["dark"]` with
  `decided: false` — dark-only by observation (see `modesConfidence`: `"dark"` is not a Plugin API
  measurement), and not yet decided to be dark-only. **The mode-flip verification in
